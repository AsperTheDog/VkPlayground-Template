...
void ArcballCamera::mouseMoved(const float p_RelX, const float p_RelY)
{
    if (m_LeftMousePressed)
    {
        const float l_Dx = p_RelX * m_RotatingSpeed;
        const float l_Dy = p_RelY * m_RotatingSpeed;
    ...
    {
        const float l_Dx = p_RelX;
        const float l_Dy = p_RelY;
...
void ArcballCamera::mouseScrolled(const float p_Y)
{
    m_DistanceToTarget -= p_Y;
